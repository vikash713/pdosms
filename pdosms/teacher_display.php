    ...
    <div class="container">
        ...
        <table border="3" cellspacing="7">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_id'])) {
                ...
                $sql = "UPDATE teacher SET fname = :fname, lname = :lname, password = :password, email = :email, phone = :phone, address = :address WHERE id = :id";
                $stmt = $db->prepare($sql);
                $params = [
                    ':fname' => $fname,
                    ':lname' => $lname,
                ...
                if ($stmt->execute($params)) {
                    ...
                    echo "<p>Error updating record: " . implode(" ", $stmt->errorInfo()) . "</p>";
                }
            }
            ...
            if (isset($_POST['delete_id'])) {
                ...
                if ($stmt->execute($params)) {
                    ...
                    echo "<p>Failed to delete record: " . implode(" ", $stmt->errorInfo()) . "</p>";
                }
            }

            // Fetch teacher records for display
            ...
            if (count($results) > 0) {
                foreach ($results as $result) {
                    echo "<tr>
                        ...
                        <td class='operations'>
                            <a href='teacherupdate_detail.php?result=" . urlencode(json_encode($result)) . "'>Edit</a>
                        </td>
                        <td class='operations'>
